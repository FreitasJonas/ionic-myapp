...
export class DocFichaPage {
  ...
  async drawComplete() {
    ...
    //var blobTest = this.imageHelper.base64toByteArray(base64, "").toString();
    var strBlob = this.imageHelper._base64ToArrayBuffer(base64);  

    let blobFromHash = this.imageHelper.base64toBlob(base64, "");

    let hash = CryptoJS.MD5(blobFromHash).toString();
    ...
    this.jObj.forEach((element, index) => {
      ...
      vetDoc.push({
        ...
        path: fullPath,
        doc: strBlob, //element.blob,
        length: strBlob.length,// / 1024, //element.blob_size,
        paginas: 1,
        hash: hash,
        extensao: "JPG",
        id_doc: index,
    ...
    var blob = this.imageHelper._base64ToArrayBuffer(block[1]);
    var fileName = vetDoc[0].protocolo + "_1.png";

    let hash2 = CryptoJS.MD5(blobFromHash).toString();
